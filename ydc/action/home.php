...
class home extends init
{
	...
	function index(){
			$title="首页";
			$base=$this->base();
			$sql="select * from ".$this->table_name("img")." where type_id='19' and type='C' and is_show='1' ";
			...
			$sql="select c.*,i.cat_name,img.original_src from ".$this->table_name("category")." as c "
				...
				." where c.cat_id in (15,16,17) and img.type='C' and img.point='0' order by order_by asc";
			...
			$sql="select g.*,i.goods_name,img.original_src from ".$this->table_name("goods")." as g "
				...
				." where g.cat_id in (11) and g.is_show='1' and img.type='P' and img.point='0' order by order_by desc";/* limit 0,4*/
			$goods=getFetchAll($sql,$this->conn);
			
			$sql="select g.*,i.goods_name,img.original_src from ".$this->table_name("goods")." as g "
				." left join ".$this->table_name("goods_i8n")." as i on i.goods_id=g.goods_id "
				." left join ".$this->table_name("img")." as img on img.type_id=g.goods_id "
				." where g.cat_id in (18) and g.is_show='1' and img.type='P' and img.point='0' order by order_by desc";/* limit 0,8*/
	...
	function contact(){
			...
			$sql="select * from ".$this->table_name("img")." ".
				" where type='A' ".$where2." order by point desc ";
